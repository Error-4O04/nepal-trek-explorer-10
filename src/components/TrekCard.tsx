...
export const TrekCard = ({ destination, isFavorite, onToggleFavorite, onBook }: TrekCardProps) => {
  ...
  // Parse duration from min/max
  const duration = destination.duration_min === destination.duration_max 
    ? `${destination.duration_min} days`
    : `${destination.duration_min}-${destination.duration_max} days`;

  // Get first 2 vibes or use empty array
  const vibes = destination.vibes && destination.vibes.length > 0 
    ? destination.vibes.slice(0, 2) 
    : [];

  // Get first 3 highlights or use empty array
  const highlights = destination.highlights && destination.highlights.length > 0
    ? destination.highlights.slice(0, 3)
    : [];

  // Average cost per person
  const avgCost = Math.round((destination.cost_budget + destination.cost_medium + destination.cost_luxury) / 3);

  return (
    <Link to={`/destination/${destination.id}`} className="group relative bg-card rounded-2xl overflow-hidden shadow-card card-hover border border-border/50 block hover:no-underline">
      {/* Image */}
      ...
      <div className="p-5 space-y-4">
        <div>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            <Badge variant="outline" className={difficultyColors[destination.difficulty]}>
        ...
        <div className="grid grid-cols-3 gap-3 py-3 border-y border-border/50">
          ...
          <div className="text-center">
            ...
            <span className="text-sm font-medium">{destination.altitude}</span>
          </div>
          <div className="text-center">
            <Leaf className={`w-4 h-4 mx-auto mb-1 ${carbonColors[destination.carbon_footprint]}`} />
            <span className="text-xs text-muted-foreground block">Carbon</span>
            <span className={`text-sm font-medium ${carbonColors[destination.carbon_footprint]}`}>
              {destination.carbon_footprint === 'low' ? '🟢' : destination.carbon_footprint === 'medium' ? '🟡' : '🔴'}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <div className="text-right">
            <span className="text-lg font-display font-bold text-primary">${avgCost}</span>
